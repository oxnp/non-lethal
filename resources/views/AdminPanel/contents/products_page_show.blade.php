                        ...
                        @foreach($product_page as $page)
                            @if($lang->id==$page->lang_id)
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-control" name="category_id[{{$page->lang_id}}][{{$page->id}}]">
                                        @foreach($categories as $cat)
                                            @if($lang->id==$cat->lang_id)
                                            <option @if($page->category_id==$cat->id) selected
                                                    @endif value="{{$cat->id}}" data-group="{{$cat->relation}}">{{$cat->name}}</option>
                                            @endif
                                        @endforeach
                                    </select>
                                    <br/><br/>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <textarea rows="7" class="form-control" name="short_text[{{$page->lang_id}}][{{$page->id}}]">{{$page->short_text}}</textarea>
                                    <br/><br/>
                                </div>
                                <div class="col-md-6">
                                    <label>Image</label>
